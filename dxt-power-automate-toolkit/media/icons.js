// ── Inline SVG icon set ─────────────────────────────────────────────────────
// A hand-picked subset of Lucide icons (lucide.dev, ISC licence) embedded as
// path data — no dependency, no build step, consistent 24×24 stroke style.
//
//   icon('trash')                → svg string for JS templates
//   <button data-icon="play">…   → static HTML; applyDataIcons() injects the
//                                  svg as the element's first child.
(function () {
  var PATHS = {
    ...
    'upload-cloud':   '<path d="M4 14.899A7 7 0 1 1 15.71 8h1.79a4.5 4.5 0 0 1 2.5 8.242"/><path d="M12 21v-9"/><path d="m16 16-4-4-4 4"/>',
  ...
  window.icon = function (name, opts) {
    ...
    return '<svg class="icon' + (opts.cls ? ' ' + opts.cls : '') + '" width="' + size + '" height="' + size +
      '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' + p + '</svg>';
  };

  // Inject icons into static HTML: <button data-icon="play">Run</button>
  window.applyDataIcons = function (root) {
    var els = (root || document).querySelectorAll('[data-icon]');
    for (var i = 0; i < els.length; i++) {
      var el = els[i];
      if (el.querySelector('svg.icon')) continue;
      el.insertAdjacentHTML('afterbegin', window.icon(el.dataset.icon));
    }
  };
})();
